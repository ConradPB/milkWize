...
export default async function paymentsRoutes(server: FastifyInstance) {
  ...
  server.post('/api/payments', async (request, reply) => {
    try {
      ...
      if (!order_id || amount == null || !method) return reply.status(400).send({ error: 'Missing order_id, amount or method' });
      if (!isValidUuid(String(order_id))) return reply.status(400).send({ error: 'order_id must be UUID' });
      ...
      const { data: adminRow } = await supabaseAdmin.from('admins').select('id').eq('auth_uid', userId).limit(1).maybeSingle();
      if (!adminRow) return reply.status(403).send({ error: 'User not mapped to admin' });

      const { data, error } = await supabaseAdmin
        .from('payments')
        .insert([{ order_id, amount, method, txn_ref, status: 'pending', paid_at: null }])
        .select();
  ...
  // Update payment status (for manual confirm or webhook use)
  server.put('/api/payments/:id', async (request, reply) => {
    try {
      const userJwt = (request.headers.authorization || '').replace('Bearer ', '').trim();
      if (!userJwt) return reply.status(401).send({ error: 'Missing JWT' });

      const { id } = request.params as any;
      if (!isValidUuid(String(id))) return reply.status(400).send({ error: 'Invalid payment id' });

      const body = (request.body || {}) as any;
      const allowed = ['status','paid_at','txn_ref'];
      const updates: any = {};
      for (const k of allowed) if (k in body) updates[k] = body[k];
      if (Object.keys(updates).length === 0) return reply.status(400).send({ error: 'No valid fields to update' });
