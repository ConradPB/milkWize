...
export default async function clientsRoutes(server: FastifyInstance) {
  ...
  import { FastifyInstance } from "fastify";
  import { supabaseAdmin } from "../supabase";
  import { isValidUuid } from "../utils"; // assume you have this helper

  export default async function clientsRoutes(server: FastifyInstance) {
    /* ---------- existing handlers (create/list/update/delete) assumed present ---------- */
    ...
    server.post("/api/clients/:id/link", async (request, reply) => {
      try {
        ...
        // Check the auth user actually exists in Supabase (optional)
        const targetUser = await supabaseAdmin.auth.getUserById(auth_user_id);
        ...
        const { data: existing, error: existErr } = await supabaseAdmin
          ...
          .maybeSingle();
        if (existErr)
          return reply.status(500).send({ error: existErr.message });
        if (existing)
          return reply
            .status(409)
    ...
    server.post("/api/clients/link-self", async (request, reply) => {
      try {
        ...
        if (!phone && !client_id)
          return reply
            .status(400)
            .send({ error: "Provide phone or client_id" });
